...
const SYS = `You are an advisor helping an EMBA student at Porto Business School choose elective courses 2025/26. Answer in Portuguese (European) unless they write English. Be concise.
RULES: 5 electives (10 ECTS). Each=2 ECTS/15h except Entrepreneurial Lab=4 ECTS/30h. Concentration=3 from same area, max 1. Sustainability Certificate=3 ESG. Min 15 students. Round 1: Apr 13-19. Round 2: May 6-13.
...
function buildReportHTML(fC,iC,fE,name,chosenConc,tECTS,track){
  ...
  const matrixCSS=`
    .mtx{border-collapse:collapse;width:100%;font-size:10px;margin-top:8px}
    .mtx th{padding:4px 3px;text-align:center;font-weight:700;font-size:8px;border:1px solid #e2e8f0;background:#f8fafc;writing-mode:vertical-lr;text-orientation:mixed;min-width:28px;height:80px}
    .mtx th.rh{writing-mode:horizontal-tb;text-align:left;font-size:10px;font-weight:500;min-width:auto;height:auto;padding:4px 8px}
    .mtx td{padding:3px;text-align:center;border:1px solid #e2e8f0;min-width:28px}
    .mtx td.rn{text-align:left;padding:4px 8px;font-size:10px;white-space:nowrap}
    .mtx .dot{display:inline-block;width:12px;height:12px;border-radius:2px;background:#2563eb}
    .mtx .dot-f{background:#059669}
    .mtx tr.totals td{font-weight:700;font-size:10px;background:#f8fafc}
    .mtx tr.totals td.hi{color:#059669}
  `;
  ...
  const renderMatrix=(rows,isFinals)=>{
    ...
    return `<table class="mtx">
      <tr><th class="rh">Elective</th>${areaShort.map((a,i)=>`<th style="color:${areaColors[i]}">${a}</th>`).join("")}</tr>
      ${rows.map(r=>`<tr>
      ...
      <tr class="totals"><td class="rn" style="font-weight:700">Total</td>${totals.map(t=>`<td class="${t>=3?"hi":""}">${t}</td>`).join("")}</tr>
    </table>`;
  };
...
${fC.length?`<h2>★ Seleção Final</h2>
${fC.map((c,i)=>`<div class="fc">
...
${iC.length?`<h2>Shortlist Completa</h2>
${ELECTIVES.map(g=>{const ig=iC.filter(c=>c.group===g.group);if(!ig.length)return"";return`<p style="font-size:12px;font-weight:600;color:${g.color};margin:10px 0 3px">${g.group}</p>
${ig.map(c=>`<div class="ii">• ${c.name} — ${c.faculty} · ${c.format}${fC.some(f=>f.id===c.id)?' <strong style="color:#059669">★ FINAL</strong>':""}</div>`).join("")}`}).join("")}`:""}

${track&&track!=="none"?`<h2>Track</h2>
${track==="entrepreneurship"?`<div class="al as" style="font-size:13px;font-weight:700">🚀 Entrepreneurship Track
...
function GuidedSelection({ candidates, alreadyFinal, onFinish, targetECTS }) {
  ...
  const slotsLeft = Math.floor(ectsLeft / 2);
  const isFull = selectedECTS >= maxECTS;
  const current = candidates[idx];
  const remaining = candidates.length - idx;

  const fetchAI = async (course, allSelected, allSkipped) => {
    setAiText(""); setAiLoading(true);
    const selNames = ALL.filter(c => allSelected.has(c.id)).map(c => c.name);
    const skipNames = ALL.filter(c => allSkipped.has(c.id)).map(c => c.name);
    const left = maxECTS - allSelected.reduce((s, id) => { const c = ALL.find(x => x.id === id); return s + (c && c.note === "= 2 electives" ? 4 : 2); }, 0);
    const prompt = `O aluno está a decidir as electives finais (objetivo: 10 ECTS). Já selecionou: ${selNames.length ? selNames.join(", ") : "nenhuma"} (${maxECTS - left} ECTS). Já excluiu: ${skipNames.length ? skipNames.join(", ") : "nenhuma"}. Faltam ${left} ECTS.
NOTA: Entrepreneurial Lab conta como 2 electives (4 ECTS). Todas as outras valem 2 ECTS cada.

Agora está a avaliar: "${course.name}" (${course.faculty}, ${course.format}, ${course.group}).
Descrição: ${course.desc}
Concentrações: ${course.concentrations.join(", ")}${course.esg ? " | ESG" : ""}

Dá uma análise BREVE (3-4 frases max) sobre se esta elective faz sentido dado o que já selecionou, concentrações possíveis, e vagas restantes. Termina com uma recomendação clara: INCLUIR ou SALTAR, e porquê em uma frase.`;
    try {
  ...
  useEffect(() => {
    if (current && !done && !selected.has(current.id)) {
      setChatMsgs([]);
      fetchAI(current, selected, skipped);
    }
  }, [idx, done]);
  ...
  const sendChat = async () => {
    ...
    const context = `Contexto: a avaliar "${current.name}". Já selecionadas: ${selNames.join(", ") || "nenhuma"}. Faltam ${slotsLeft} vagas.`;
  ...
  if (done) {
    ...
    const ects = finalList.reduce((s, c) => s + (c.note === "= 2 electives" ? 4 : 2), 0);
    return (
      <div style={{ padding: 24, maxWidth: 600, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 36, marginBottom: 8 }}>✅</div>
          <h2 style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: 22, fontWeight: 400, margin: "0 0 4px" }}>Seleção Completa</h2>
          <p style={{ fontSize: 13, color: "#64748b" }}>{selected.size} electives · {ects} ECTS</p>
        </div>
        {finalList.map((c, i) => (
          <div key={c.id} style={{ padding: "10px 14px", borderRadius: 8, borderLeft: "3px solid #059669", background: "#f0fdf4", marginBottom: 6 }}>
            <div style={{ fontWeight: 600, fontSize: 14 }}>{i + 1}. {c.name}</div>
            <div style={{ fontSize: 11, color: "#64748b" }}>{c.faculty} · {c.format} · {c.group}</div>
          </div>
        ))}
        <div style={{ display: "flex", gap: 8, marginTop: 16, justifyContent: "center" }}>
          <button onClick={() => onFinish(selected)} style={{ padding: "8px 20px", borderRadius: 8, border: "none", background: "#059669", color: "#fff", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Confirmar seleção</button>
          <button onClick={() => { setDone(false); setIdx(0); setSelected(new Set(alreadyFinal)); setSkipped(new Set()); }} style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", color: "#64748b", fontSize: 13, cursor: "pointer" }}>Recomeçar</button>
          <button onClick={() => onFinish(null)} style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", color: "#64748b", fontSize: 13, cursor: "pointer" }}>Voltar</button>
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", height: "100%", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <button onClick={() => onFinish(null)} style={{ background: "none", border: "none", color: "#64748b", fontSize: 12, cursor: "pointer" }}>← Voltar à triagem</button>
          <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
            <span style={{ color: "#059669", fontWeight: 700 }}>★ {selectedECTS}/{maxECTS} ECTS</span>
            <span style={{ color: "#94a3b8" }}>{remaining} por avaliar</span>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", gap: 3, marginBottom: 6 }}>
            {Array.from({ length: 5 }).map((_, i) => {
              ...
              for (const id of filled) {
                ...
                if (!c) continue;
                const cEcts = c.note === "= 2 electives" ? 4 : 2;
                if (ectsUsed + cEcts > i * 2 && ectsUsed <= i * 2) slotCourse = c;
                ectsUsed += cEcts;
              }
              const isFilled = i * 2 < selectedECTS;
              return (
                <div key={i} style={{ flex: 1, padding: "5px 6px", borderRadius: 6, border: isFilled ? "2px solid #059669" : "2px dashed #d1d5db", background: isFilled ? "#f0fdf4" : "#fafafa", minHeight: 28, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <span style={{ fontSize: 9, color: isFilled ? "#059669" : "#cbd5e1", fontWeight: 600, textAlign: "center", lineHeight: 1.1 }}>{isFilled ? `${(i + 1) * 2} ECTS` : `${(i + 1) * 2}`}</span>
                </div>
              );
            })}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {ALL.filter(c => selected.has(c.id)).map(c => (
              <span key={c.id} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: "#d1fae5", color: "#065f46", fontWeight: 600 }}>
                {c.name} ({c.note === "= 2 electives" ? "4" : "2"} ECTS)
              </span>
            ))}
          </div>
        </div>
        ...
        {/* Current card */}
        <div style={{ border: "2px solid #2563eb", borderRadius: 12, padding: 16, background: "#eff6ff", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
            <span style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: 18, fontWeight: 600, color: "#1e293b" }}>{current.name}</span>
            {current.esg && <span style={{ fontSize: 9, fontWeight: 700, background: "#d1fae5", color: "#065f46", padding: "1px 5px", borderRadius: 3 }}>ESG</span>}
            {current.note && <span style={{ fontSize: 9, fontWeight: 600, background: "#fef9c3", color: "#854d0e", padding: "1px 5px", borderRadius: 3 }}>{current.note}</span>}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center", marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: "#64748b" }}>{current.faculty}</span>
            {current.links.map((l, i) => <a key={i} href={l} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: "#2563eb" }}>LinkedIn ↗</a>)}
            <span style={{ fontSize: 10, fontWeight: 600, padding: "1px 6px", borderRadius: 3, background: b.bg, color: b.tx }}>{current.format}</span>
            <span style={{ fontSize: 10, color: "#94a3b8" }}>{current.group}</span>
          </div>
          <p style={{ fontSize: 12, color: "#475569", lineHeight: 1.5, marginBottom: 6 }}>{current.desc}</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
            {current.concentrations.map(c => <span key={c} style={{ fontSize: 9, padding: "1px 5px", borderRadius: 3, background: "#e2e8f0", color: "#475569" }}>{c}</span>)}
          </div>
        </div>

        {/* AI recommendation */}
        <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 12, background: "#f8fafc", marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#5b21b6", marginBottom: 6 }}>🤖 Análise do Agente</div>
          {aiLoading ? <div style={{ fontSize: 12, color: "#94a3b8" }}>A analisar...</div>
            : <div style={{ fontSize: 12, color: "#1e293b", lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{aiText}</div>}
        </div>
        ...
        {(() => {
          ...
          return (
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexDirection: "column", alignItems: "center" }}>
              {wouldExceed && !isFull && <div style={{ fontSize: 11, color: "#b45309", marginBottom: 4 }}>⚠ Esta elective vale {courseECTS} ECTS — excederia o limite de ${maxECTS} ECTS ({selectedECTS} + {courseECTS} = {selectedECTS + courseECTS})</div>}
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={handleSelect} disabled={aiLoading || cantAdd} style={{
                  padding: "10px 28px", borderRadius: 8, border: "none", cursor: aiLoading || cantAdd ? "not-allowed" : "pointer",
                  background: cantAdd ? "#cbd5e1" : "#059669", color: "#fff", fontWeight: 700, fontSize: 14,
                }}>✓ Incluir ({courseECTS} ECTS)</button>
                <button onClick={handleSkip} disabled={aiLoading} style={{
                  padding: "10px 28px", borderRadius: 8, border: "1px solid #d1d5db", cursor: aiLoading ? "wait" : "pointer",
      ...
      {/* Chat sidebar */}
      <div style={{ width: 300, borderLeft: "1px solid #e2e8f0", display: "flex", flexDirection: "column", background: "#fff" }}>
        <div style={{ padding: "8px 10px", borderBottom: "1px solid #e2e8f0", fontSize: 11, fontWeight: 600 }}>💬 Perguntas sobre "{current.name}"</div>
        <div style={{ flex: 1, overflowY: "auto", padding: "8px 10px", display: "flex", flexDirection: "column", gap: 6 }}>
          {chatMsgs.length === 0 && <div style={{ color: "#94a3b8", fontSize: 11, textAlign: "center", marginTop: 16 }}>Tens dúvidas?<br />Pergunta antes de decidir.</div>}
          {chatMsgs.map((m, i) => <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "90%", padding: "6px 9px", borderRadius: 8, background: m.role === "user" ? "#2563eb" : "#f1f5f9", color: m.role === "user" ? "#fff" : "#1e293b", fontSize: 11.5, lineHeight: 1.4, whiteSpace: "pre-wrap" }}>{m.content}</div>)}
          {chatLoading && <div style={{ fontSize: 11, color: "#94a3b8" }}>A pensar...</div>}
          <div ref={chatEnd} />
        </div>
        <div style={{ borderTop: "1px solid #e2e8f0", padding: 6, display: "flex", gap: 4 }}>
          <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && (e.preventDefault(), sendChat())} placeholder="Pergunta..." style={{ flex: 1, padding: "5px 8px", borderRadius: 6, border: "1px solid #d1d5db", fontSize: 11, outline: "none" }} />
          <button onClick={sendChat} disabled={chatLoading || !chatInput.trim()} style={{ padding: "5px 10px", borderRadius: 6, border: "none", background: chatLoading || !chatInput.trim() ? "#cbd5e1" : "#2563eb", color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>→</button>
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  const changeTrack=(t)=>{
    ...
    if(newTrack==="entrepreneurship"){
      // Auto-add ELab as final and interested
  ...
  const unlockedConcs=CONC_AREAS.filter(a=>concCountsFinal[a]>=3);
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px 10px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:8,padding:"8px 10px",borderRadius:8,background:"#f8fafc",border:"1px solid #e2e8f0"}}>
          <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
            ...
            {[5,6,7].map(n=>(
              <button key={n} onClick={()=>setTargetN(n)} style={{
                padding:"3px 10px",borderRadius:5,cursor:"pointer",fontSize:10,fontWeight:targetN===n?700:400,
                border:targetN===n?"2px solid #2563eb":"1px solid #d1d5db",
                background:targetN===n?"#eff6ff":"#fff",color:targetN===n?"#2563eb":"#64748b",
              }}>{n} ({n*2} ECTS)</button>
            ))}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
        ...
        <div style={{marginBottom:8,padding:"6px 8px",borderRadius:6,background:"#f8fafc",border:"1px solid #e2e8f0"}}>
          <div style={{display:"flex",flexWrap:"wrap",gap:3,alignItems:"center"}}>
            ...
            {(()=>{const unlocked=esgCount>=3;
              return(<span key="esg" style={{fontSize:8,padding:"2px 6px",borderRadius:3,fontWeight:700,background:unlocked?"#d1fae5":esgCount>0?"#f1f5f9":"#f1f5f9",color:unlocked?"#059669":esgCount>0?"#065f46":"#cbd5e1",border:unlocked?"1.5px solid #059669":"none",transition:"all .2s"}}>{unlocked?"🔓 ":""}ESG {esgCount}/3</span>);
            })()}
          </div>
